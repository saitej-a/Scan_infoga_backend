...
def get_corporate_dataset_dynamodb_table(table_name=None):
    if dynamodb_resource is None:
        initialize_dynamodb_clients()
        if dynamodb_resource is None:
            raise Exception("DynamoDB resource is not initialized. Check AWS configuration.")
...
def get_zomato_dataset_dynamodb_table(table_name=None):
    if dynamodb_resource is None:
        initialize_dynamodb_clients()
        if dynamodb_resource is None:
            raise Exception("DynamoDB resource is not initialized. Check AWS configuration.")
...
def get_cbse_dataset_dynamodb_table(table_name=None):
    if dynamodb_resource is None:
        initialize_dynamodb_clients()
        if dynamodb_resource is None:
            raise Exception("DynamoDB resource is not initialized. Check AWS configuration.")
    if table_name is None:
        table_name = settings.DYNAMODB_CBSE_TABLE_NAME

    return dynamodb_resource.Table(table_name)

def get_olx_dataset_dynamodb_table(table_name=None):
    if dynamodb_resource is None:
        initialize_dynamodb_clients()
        if dynamodb_resource is None:
            raise Exception("DynamoDB resource is not initialized. Check AWS configuration.")
    if table_name is None:
        table_name = settings.DYNAMODB_OLX_TABLE_NAME
