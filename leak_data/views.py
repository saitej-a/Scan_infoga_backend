...
from core.exception_handlers import DataNotFoundException, InsufficientBalanceError, MissingRequiredFieldsException
...
def get_password(request):
    ...
    if not email:
        ...
        raise MissingRequiredFieldsException("Email is required in the request body.")
    if not isinstance(email, str):
        # return Response(create_response(False, "Email must be a string.", None), status=status.HTTP_400_BAD_REQUEST)
        raise MissingRequiredFieldsException("Email must be a string.")
    ...
    if item:
        ...
        raise DataNotFoundException("Data not found in database.")
...
def get_job_seeker_data(request):
    ...
    if not mobno:
        ...
        raise MissingRequiredFieldsException("Mobile number is required in the request body.")
    if not isinstance(mobno, str):
        # return Response(create_response(False, "Mobile number must be a string.", None), status=status.HTTP_400_BAD_REQUEST)
        raise MissingRequiredFieldsException("Mobile number must be a string.")

    if len(mobno) != 10:
        # return Response({"error": "Mobile number must be 10 digits."}, status=status.HTTP_400_BAD_REQUEST)
        raise MissingRequiredFieldsException("Mobile number must be 10 digits.")
...
def get_olx_data(request):
    ...
    if len(mobno) != 10:
        # return Response(create_response(False, "Mobile number must be 10 digits.", None),status=status.HTTP_400_BAD_REQUEST)
        raise MissingRequiredFieldsException("Mobile number must be 10 digits.")
    ...
    if items:
        ...
        #     status=status.HTTP_200_OK
        # )
        raise DataNotFoundException("Mobile number not found in the database.")
...
def get_india_mart_data(request):
    ...
    if not request_body:
        ...
        #     status=status.HTTP_400_BAD_REQUEST
        # )
        raise MissingRequiredFieldsException("Request body is required.")
    ...
    if re.match(email_regex, input_str):
        ...
        #     status=status.HTTP_400_BAD_REQUEST
        # )
        raise MissingRequiredFieldsException("Invalid input. Provide a valid email or 10-digit Indian mobile number.")
...
def process_india_mart_results(items, user, is_called, request, not_found_msg):
    if items:
        ...
        #     status=status.HTTP_200_OK
        # )
        raise DataNotFoundException(not_found_msg)
